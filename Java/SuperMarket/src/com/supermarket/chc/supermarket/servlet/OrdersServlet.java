package com.supermarket.chc.supermarket.servlet;

import com.supermarket.chc.supermarket.biz.GoodsBiz;
import com.supermarket.chc.supermarket.biz.OrdersBiz;
import com.supermarket.chc.supermarket.biz.SuppliersBiz;
import com.supermarket.chc.supermarket.biz.impl.GoodsBizImpl;
import com.supermarket.chc.supermarket.biz.impl.OrdersBizImpl;
import com.supermarket.chc.supermarket.biz.impl.SuppliersBizImpl;
import com.supermarket.chc.supermarket.entity.*;
...
public class OrdersServlet extends HttpServlet {
	...
	private void process(HttpServletRequest request,
		...
		int utyoe = (int)session.getAttribute("utype");//用户类别
		if (action == null) {
			...
			if(printWaring("订单更新",request,response,session)){
			...
			if (printWaring("订单删除",request,response,session)){
				//
			}else{
				...
				// 获得Cookie
				Cookie[] cookies = request.getCookies();//从Cookie获取oid
				if (cookies.length > 0) {
	...
	private boolean printWaring(String waring, HttpServletRequest request,
		...
		if(utype == 3){
			...
			out.print("<script>alert('普通用户没有权限进行"+waring+"'); window.location='security/billList.jsp' </script>");
			out.flush();
			out.close();
